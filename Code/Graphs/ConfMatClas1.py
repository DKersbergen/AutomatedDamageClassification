...
out = open("output/Conf_mat_class1.txt", "a") 

keyList = df.keys().tolist()
detThresList = np.around(np.linspace(0.0, 0.94, 18), decimals=2)
counter = 0
for key in keyList:
    if key[-4:] == "mean" or key[-6:] == "median":
        if counter < 1:
            for thres in detThresList:
                detThres1 = np.around(np.linspace(thres, 0.96, int((1.0-thres)*19)), decimals=2)
                for thres1 in detThres1:                
                    detThres2 = np.around(np.linspace(thres1, 0.98, int((1.0-thres1)*20)), decimals=2)
                    for thres2 in detThres2:
                        detThres3 = np.around(np.linspace(thres2, 1.0, int((1.0-thres2)*21)), decimals=2)
                        for thres3 in detThres3:
                            print(thres, thres1, thres2, thres3)
                            counter += 1
                            condis = [
                                (df[key] <= thres ), (df[key] > thres) & (df[key] <= thres1),
                                (df[key] > thres2) & (df[key] <= thres3), (df[key] > thres3)]
                            choics = ['none', 'partial', 'significant', 'destroyed']
                            dfstring = key + "_" + str(thres) +"_" + str(thres1) +"_" + str(thres2) +"_" + str(thres3)
                            ...
                            counter+=1
                            print(counter)
                            with open("output/CohenScore_class1.csv", "a", newline='') as csvfile:
