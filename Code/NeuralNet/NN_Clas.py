...
for name in no_names:
    if int(name[:-6]) in listofEx:
        predictSet.append(np.float64(imread(dir_no + name, mode="RGB")))
        predictName.append(name[:-6])
    else:
        no_images.append(np.float64(imread(dir_no + name, mode="RGB")))
for name in min_names:
    if int(name[:-6]) in listofEx:
        predictSet.append(np.float64(imread(dir_min + name, mode="RGB")))
        predictName.append(name[:-6])
    else:
        min_images.append(np.float64(imread(dir_min + name, mode="RGB")))
for name in sig_names:
    if int(name[:-6]) in listofEx:
        predictSet.append(np.float64(imread(dir_sig + name, mode="RGB")))
        predictName.append(name[:-6])
    else:
        sig_images.append(np.float64(imread(dir_sig + name, mode="RGB")))
for name in dest_names:
    if int(name[:-6]) in listofEx:
        predictSet.append(np.float64(imread(dir_dest + name, mode="RGB")))
        predictName.append(name[:-6])
    else:
        dest_images.append(np.float64(imread(dir_dest + name, mode="RGB")))
...
convnet = input_data(shape=[None, 100, 100, 3], name='input', data_preprocessing=img_prep)
...
convnet = fully_connected(convnet, 4, activation='softmax',weights_init='normal')
...
model.fit({'input': X}, {'targets': Y}, n_epoch=35, validation_set=({'input': test_x}, {'targets': test_y}), 
    snapshot_step=99, show_metric=True, run_id="DamClass")
model.save('CNN_damage_Class_1.tflearn')
"""
model.load('CNN_damage_Class_1.tflearn')
with open('prediction_class.csv', 'a', newline='') as outfile:
