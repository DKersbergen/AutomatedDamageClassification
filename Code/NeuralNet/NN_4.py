...
for name in min_names:
    if int(name[:-6]) in listofEx:
        ...
        no_images.append(np.float64(imread(dir_min + name, mode="RGB")))
for name in sig_names:
    if int(name[:-6]) in listofEx:
        predictSet.append(np.float64(imread(dir_sig + name, mode="RGB")))
        predictName.append(name[:-6])
    else:
        dam_images.append(np.float64(imread(dir_sig + name, mode="RGB")))
for name in dest_names:
    if int(name[:-6]) in listofEx:
        predictSet.append(np.float64(imread(dir_dest + name, mode="RGB")))
        predictName.append(name[:-6])
    else:
        dam_images.append(np.float64(imread(dir_dest + name, mode="RGB")))
...
model.fit({'input': X}, {'targets': Y}, n_epoch=35, validation_set=({'input': test_x}, {'targets': test_y}), 
    snapshot_step=99, show_metric=True, run_id="DamDect")
model.save('CNN_damage_detection_3.tflearn')
"""
model.load('CNN_damage_detection_3.tflearn')
with open('prediction_3.csv', 'a', newline='') as outfile:
